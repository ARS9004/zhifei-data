...
- 调试信息直接显示在界面
- BAILIAN_APP_ID 已写死，无需配置 Secrets
"""
...
附加参考: {hint}"""
    ...
    st.write("--- 百炼应用API调用 ---")
    st.write(f"应用ID: {BAILIAN_APP_ID}")
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            raise RuntimeError(f"❌ 百炼应用API调用失败 (HTTP {resp.status_code}): {error_detail}")

        result = resp.json()
        output = result.get("output", {})
        choices = output.get("choices", [])
        if not choices:
            raise RuntimeError("❌ 百炼应用API返回为空")
        ...
        raise RuntimeError("❌ 百炼应用API请求超时（120秒）")
    except Exception as e:
