...
- 文件上传：系统原生文件选择器
- 导出：公众号发布级格式
- 【云端专版】所有记忆读写走 RDS（chat_memory 表），不依赖本地 SQLite
- 底部布局：输入框 + 📎文件按钮同行
- 输出直接展开，无折叠
- 历史对话加载：最近10轮（20条消息）
- DEBUG: 打印百炼请求入参，排查 tools 是否传递
"""
...
# ================= 云端版记忆库（纯 RDS，不依赖本地 SQLite） =================
def init_memory_db():
    """云端版：禁用本地 SQLite 初始化，只记录日志"""
    logger.info("☁️ 云端模式：记忆库使用 RDS，不初始化本地 SQLite")
    pass

def save_memory(u: str, a: str):
    """写入 RDS chat_memory 表"""
    try:
...
def load_history_from_rds(limit: int = 10) -> List[Dict[str, str]]:
    """从 RDS 加载最近 N 轮对话，返回 messages 格式列表"""
    try:
        conn = get_rds_connection()
        with conn.cursor() as cursor:
            cursor.execute(
...
附加参考: {hint}"""
    ...
    # ================= DEBUG: 打印完整请求入参 =================
    print(f"[DEBUG] ========== 百炼请求开始 ==========")
    print(f"[DEBUG] 模型名称: {MODEL_NAME}")
    print(f"[DEBUG] System Prompt 长度: {len(sys_p)}")
    print(f"[DEBUG] 消息数量: {len(full_msgs)}")
    print(f"[DEBUG] 前3条消息预览: {json.dumps(full_msgs[:3], ensure_ascii=False, default=str)[:500]}")
    print(f"[DEBUG] 是否包含 tools 字段: 当前代码未传递 tools 参数")
    print(f"[DEBUG] ========== 百炼请求结束 ==========")
