...
if not BAILIAN_APP_ID:
    raise RuntimeError("⛔ 请配置环境变量 BAILIAN_APP_ID")
...
附加参考: {hint}"""
    ...
    full_messages = [{"role": "system", "content": sys_p}]
    for m in messages:
        role = m["role"]
    ...
    # 调试信息直接显示在界面
    st.write("--- 百炼应用 API 调用 ---")
    st.write(f"AppId: {BAILIAN_APP_ID}")
    st.write(f"消息数: {len(full_messages)}")
    st.write("--- 调用中... ---")
    ...
    try:
        ...
        raise RuntimeError("❌ 百炼 API 请求超时（60秒）")
    except Exception as e:
        raise RuntimeError(f"❌ 请求异常: {e}")

    if resp.status_code != 200:
        error_detail = resp.text
        raise RuntimeError(f"❌ 百炼 API 调用失败 (HTTP {resp.status_code}): {error_detail}")

    result = resp.json()
    output = result.get("output", {})
    if output.get("finish_reason") == "error":
        error_msg = output.get("text", "未知错误")
        raise RuntimeError(f"❌ 百炼 API 返回错误: {error_msg}")

    choices = output.get("choices", [])
    if not choices:
        raise RuntimeError("❌ 百炼 API 返回为空")
    ...
    reset_health_status()
    total_tok = estimate_tokens(sys_p) + estimate_messages_tokens(messages) + estimate_tokens(full_text)
